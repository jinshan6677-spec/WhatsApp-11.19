  ...
  buildPrompt(text, sourceLang, targetLang, style) {
    const stylePrompts = {
      ...
      '正式': {
        ...
        emphasis: '风格要求：使用正式、庄重的商务语气。采用敬语、完整句式，避免缩写和口语化表达。保持专业、严谨、得体的语言风格。'
      },
      '口语化': {
        instruction: '你是一个日常对话翻译助手。请将以下文本翻译成{targetLang}。',
        emphasis: '风格要求：使用轻松、随意的日常口语风格。可以使用缩写、常见俚语、语气词，就像朋友之间聊天一样自然亲切。避免过于正式的书面语。'
      },
      '亲切': {
        instruction: '你是一个温暖友好的翻译助手。请将以下文本翻译成{targetLang}。',
        emphasis: '风格要求：使用亲切、温暖、关怀的语气。采用柔和的表达方式，让人感到被关心和重视。保持友善、体贴的语言风格。'
      },
      '幽默': {
        instruction: '你是一个风趣的翻译助手。请将以下文本翻译成{targetLang}。',
        emphasis: '风格要求：使用轻松、风趣、俏皮的表达方式。在忠实原文意思的前提下，可以选择更生动、更有趣的词汇和表达。如果原文有双关语、俏皮话或幽默元素，要巧妙地在译文中体现。但注意：不要添加原文没有的笑话或幽默内容，只是让表达方式更轻松有趣。'
      },
      '礼貌': {
        instruction: '你是一个注重礼仪的翻译助手。请将以下文本翻译成{targetLang}。',
        emphasis: '风格要求：使用礼貌、尊重、谦逊的语气。多用敬语、委婉表达，体现良好教养和对他人的尊重。'
      },
      '强硬': {
        instruction: '你是一个翻译助手。请将以下文本翻译成{targetLang}。',
        emphasis: '风格要求：使用坚定、有力、果断的语气。用词直接、明确，展现权威性和决断力。避免模棱两可的表达。'
      },
      '简洁': {
        instruction: '你是一个翻译助手。请将以下文本翻译成{targetLang}。',
        emphasis: '风格要求：使用最简洁明了的表达。去除冗余词汇，用最少的字传达完整意思。追求精炼、直接的表达方式。'
      },
      '激励': {
        instruction: '你是一个翻译助手。请将以下文本翻译成{targetLang}。',
        emphasis: '风格要求：如果原文本身带有激励性质，请在翻译时保持并适当体现这种积极向上的风格。如果原文是中性内容，请忠实翻译，不要添加激励元素。'
      },
      '中立': {
        instruction: '你是一个客观中立的翻译助手。请将以下文本翻译成{targetLang}。',
        emphasis: '风格要求：保持中立、客观、不带感情色彩。避免任何主观评价或情绪化表达，保持理性和公正的语言风格。'
      },
      '专业': {
        instruction: '你是一个专业领域的翻译专家。请将以下文本翻译成{targetLang}。',
        emphasis: '风格要求：使用专业、技术性、学术化的表达方式。优先使用行业术语和规范表述，体现专业水准。'
      }
    };
